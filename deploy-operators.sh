...
kubectl apply -k nfd-operator/base/
echo "   Waiting for NFD Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=nfd -n openshift-nfd --timeout=300s

echo "   Applying NFD Overlay (CRDs)..."
kubectl apply -k nfd-operator/overlays/crds/
echo "   Waiting for NFD instance to be Available..."
kubectl wait --for=jsonpath='{.status.conditions[?(@.type=="Available")].status}'=True nfd-instance -n openshift-nfd --timeout=300s

echo "2. Deploying NVIDIA GPU Operator..."
kubectl apply -k nvidia-operator/base/
echo "   Waiting for NVIDIA GPU Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=gpu-operator -n nvidia-gpu-operator --timeout=300s

echo "   Applying NVIDIA GPU Operator Overlay (CRDs)..."
kubectl apply -k nvidia-operator/overlays/crds/
echo "   Waiting for GPU ClusterPolicy to be ready..."
kubectl wait --for=jsonpath='{.status.state}'=ready clusterpolicy gpu-cluster-policy --timeout=300s

# Phase 2: Platform Operators
echo "--- Phase 2: Deploying Platform Operators ---"

echo "3. Deploying OpenShift Serverless Operator..."
kubectl apply -k serverless-operator/base/
echo "   Waiting for OpenShift Serverless Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=serverless-operator -n openshift-serverless --timeout=300s

echo "4. Deploying OpenShift ServiceMesh Operator..."
kubectl apply -k servicemesh-operator/base/
echo "   Waiting for OpenShift ServiceMesh Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=servicemeshoperator -n openshift-operators --timeout=300s

echo "5. Deploying Authorino Operator..."
kubectl apply -k authorino-operator/base/
echo "   Waiting for Authorino Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=authorino-operator -n openshift-operators --timeout=300s
...
kubectl apply -k rhoai-operator/base/
echo "   Waiting for RHOAI Operator to be ready..."
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=rhods-operator -n redhat-ods-operator --timeout=300s

echo "   Applying RHOAI Operator Overlay (CRDs)..."
kubectl apply -k rhoai-operator/overlays/crds/
echo "   Waiting for DataScienceCluster to be ready..."
kubectl wait --for=jsonpath='{.status.conditions[?(@.type=="Ready")].status}'=True datasciencecluster default-dsc --timeout=600s
